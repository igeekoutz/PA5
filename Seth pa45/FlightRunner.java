...
public class FlightRunner {
    ...
    public static String[] findClient(HashMap<String, Customer> allCustomers, HashMap<String, Employee> allEmployees,
            String[] name) {
        for (String key : allCustomers.keySet()) {
            if (allCustomers.get(key).getFirstName().equalsIgnoreCase(name[0])
                    && allCustomers.get(key).getLastName().equalsIgnoreCase(name[1])) {
                String[] importantInfo = { allCustomers.get(key).getRole(), allCustomers.get(key).getUserName() };
                return importantInfo;
            }
        }
        for (String key : allEmployees.keySet()) {
            if (allEmployees.get(key).getFirstName().equalsIgnoreCase(name[0])
                    && allEmployees.get(key).getLastName().equalsIgnoreCase(name[1])) {
    ...
    /**
     * Reads a file containing actions for automatic ticket purchasing and executes
     ...
     */
    public static void automaticTicketPurchasing(HashMap<String, Customer> allCustomers,
            HashMap<String, Flight> allFlights, HashMap<String, Employee> allEmployees) throws FileNotFoundException {
        System.out.println(
                "Welcome to the Automatic ticket purchaser, there are 3 options\n\n1. AutoPurchaser10k\n2.AutoPurchaser100k\n3.AutoPurchaser400k\n4. Go Back\n");
        String selection = scan.next();
        switch (selection) {
            case "1":
                Scanner autoPurchaser10k = new Scanner(new File("AutoPurchaser10K.csv"));
                while (autoPurchaser10k.hasNextLine()) { // This while loop populates allFlight hash map with all flight
                                                         // data
            ...
            case "2":
                ...
                while (autoPurchaser100k.hasNextLine()) { // This while loop populates allFlight hash map with all
                                                          // flight data
            ...
            case "3":
                ...
                while (autoPurchaser400k.hasNextLine()) { // This while loop populates allFlight hash map with all
                                                          // flight data
    ...
    /**
     * 
     * This method allows the user to view airport information by airport code. The
     * user will be prompted to enter an airport code and the information regarding
     * the airport will be printed including the airport name, city, state, country,
     * fees and lounge status. The user may choose to view information for another
     * airport or exit the menu.
     * 
     ...
     * @param importantInfo An array containing important information about the
     *                      currently logged in user, such as their name and role
     * @throws NoSuchElementException If the scanner is closed or empty when trying
     *                                to read user input
     * @return void This method does not return anything
     */
    public static void viewAirportInformation(HashMap<String, Customer> allCustomers,
            HashMap<String, Flight> allFlights, HashMap<String, Employee> allEmployees, String[] importantInfo) {
        boolean continue1 = true;
        while (continue1) {
            System.out.println(
                    "Here you may view Airport Information by airport code, to begin please eneter an Airport code");
            System.out.print("\nAirport Code: ");
            String selection = scan.next();
    ...
    /**
     * 
     * This method prints a menu allowing customers to search for flight information
     * based on flight ID or flights between origin and destination airport.
     * If the customer selects option 1, they will be prompted to enter the ID of
     * the flight they want information for.
     * If the customer selects option 2, they will be prompted to enter the origin
     * and destination codes and all flights between those two airports will be
     * displayed.
     * After either option, the customer will be prompted to purchase a ticket for
     * the selected flight.
     * 
     * @param allCustomers  A HashMap containing all customers.
     * @param allFlights    A HashMap containing all flights.
     * @param allEmployees  A HashMap containing all employees.
     * @param importantInfo An array of Strings containing important information for
     *                      the program.
     */
    public static void viewFlightInformationMenu(HashMap<String, Customer> allCustomers,
            HashMap<String, Flight> allFlights, HashMap<String, Employee> allEmployees, String[] importantInfo) {
        System.out.println(
                "How would you like to serach for a Flight(s)?\n\n1. Flight ID\n2. Flights between 'X' and 'Y'\n");
        ...
        switch (selection) {
            ...
            case "2":
                System.out.print(
                        "To see flights between 'Origin Code' and 'Destination Code' please eneter both codes\n\n Origin Code: ");
                String originCode = scan.next();
                System.out.println("\nDestination Code: ");
                String destinationCode = scan.next();
                for (String key : allFlights.keySet()) {

                    if (originCode.equalsIgnoreCase(allFlights.get(key).getOriginAirport().getOriginCode())
                            && destinationCode.equalsIgnoreCase(
                                    allFlights.get(key).getDestinationAirport().getDestinationCode())) {
                        System.out.println(line);
                ...
                System.out
                        .println("Would you like to purchase a ticket for any of these flights?\n\n 1. Yes\n 2. No\n");
                System.out
                        .println("\n*PLEASE WRITE DOWN THE FLIGHT ID YOUD LIKE TO BUY TICKETS FOR BEFORE MOVING ON*\n");
    ...
    /**
     * 
     * This method displays a cancellation menu for customers and employees to
     * cancel a ticket for a flight.
     * If the user is a customer, they will be shown a list of their available
     * tickets and asked to enter the
     * confirmation number of the ticket they would like to cancel. The ticket will
     * then be cancelled and the
     * customer will be refunded. If the user is an employee, they will be asked to
     * enter the ticket confirmation
     ...
     * @param importantInfo an array containing important user information, such as
     *                      user type and ID
     */
    public static void cancelTicketMenu(HashMap<String, Customer> allCustomers, HashMap<String, Flight> allFlights,
            HashMap<String, Employee> allEmployees, String[] importantInfo) {
        if (importantInfo[0].equalsIgnoreCase("Customer")) {
            ...
            while (stayin2) {
                ...
                addToList("Customer " + allCustomers.get(importantInfo[1]).getFirstName() + " "
                        + allCustomers.get(importantInfo[1]).getLastName() + " cancelled a ticket for Flight #"
                        + ticketID + " money has been refunded and ticket set to inactive");
            ...
            boolean stayIn2 = true;
            while (stayIn2) {
                // itetrate over all of the tickets in flight returning the ticket total to the
                // username attached to the ticket, checkl what type of seats
                // were purchased and return that amount to that sepcifi type and to total seats
                // as well, finally, set the active parameter inb the tciket to false
                ...
                System.out.println("Would you like to cancel the tickets for another flight?\n\n1. Yes\n2. No\n");
                addToList("Employee " + allEmployees.get(importantInfo[1]).getFirstName() + " "
                        + allEmployees.get(importantInfo[1]).getLastName() + " cancelled all tickets for Flight #"
                        + flightToCancel + " all money has been refunded and tickets set to inacvtive");
    ...
    /**
     * 
     * This method adds up all the fees for a given flight, including Miner Airlines
     * fee, security fee, and taxes.
     * 
     ...
     * @return the new total cost of the transaction with the added Miner Airlines
     *         fee
     */
    ...
    public static double minerAirlinesMember(HashMap<String, Customer> allCustomers, HashMap<String, Flight> allFlights,
            HashMap<String, Employee> allEmployees, String[] importantInfo, double total, Flight currFlight) {
        if (importantInfo[0].equalsIgnoreCase("Customer")) {
    ...
    public static void purchaseTicketMenu(HashMap<String, Customer> allCustomers, HashMap<String, Flight> allFlights,
            HashMap<String, Employee> allEmployees, String[] importantInfo, boolean automaticPurchasing,
            String[] actions) {
        if (automaticPurchasing == false) {
            ...
            System.out.println("1. First Class Price: $" + allFlights.get(inputID).getFirstClassPrice()
                    + " Seats available: " + allFlights.get(inputID).getFirstClassSeats()
                    + "\n2. Business Class Price: $" + allFlights.get(inputID).getBusinessClassPrice()
                    + " Seats available: " + allFlights.get(inputID).getBusinessClassSeats()
                    + "\n3. Main Cabin Price: $" + allFlights.get(inputID).getMainCabinPrice() + " Seats available: "
                    + allFlights.get(inputID).getMainCabinSeats());
            System.out.println();
            if (importantInfo[0].equalsIgnoreCase("Customer")) {
            ...
            if (Integer.parseInt(ticketType) == 1) {
                System.out.println(
                        "How many First Class Tickets would you like to purchase? (You can purchase a maximum of 8 at a time)");
                int ticketsPurchased = scan.nextInt();
                if (ticketsPurchased > 8) {
                    while (ticketsPurchased > 8) {
                        System.out
                                .print("You tried to purchase more than 8 tickets, please enter a different amount: ");
                ...
                if (importantInfo[0].equalsIgnoreCase("Customer")) {
                    ...
                    newTotal = minerAirlinesMember(allCustomers, allFlights, allEmployees, importantInfo, newTotal,
                            allFlights.get(inputID));
                    System.out.println(
                            "Your total comes out to: " + newTotal + " Would you like to proceed?\n\n1. Yes\n2. No\n");
                    int confirmPurchase = scan.nextInt();
                    if (allCustomers.get(importantInfo[1]).hasEnoughMoney(newTotal) && confirmPurchase == 1) { // This
                                                                                                               // is
                                                                                                               // used
                                                                                                               // to
                                                                                                               // check
                                                                                                               ...
                                                                                                               // enough
                                                                                                               // money
                        if (allFlights.get(inputID).checkSeats("First Class", ticketsPurchased,
                                allFlights.get(inputID))) { // if theres enough seats avaailable then proceed
                            confirmationNumber += 1;
                            String confirmNumString = String.valueOf(confirmationNumber);
                            Ticket newTicket = new Ticket(newTotal, ticketsPurchased, confirmNumString,
                                    allFlights.get(inputID).getID(), importantInfo[1], "First Class", true); // Create a
                                                                                                             // new
                                                                                                             // ticket
                                                                                                             // given
                                                                                                             // the
                                                                                                             ...
                                                                                                             // enetred
                                                                                                             // amounts
                            allFlights.get(inputID).addTicket(newTicket); // add ticket to public variable allTickets
                                                                          // which contains all purchased tickets
                            allCustomers.get(importantInfo[1]).addTicket(newTicket); // Add tickets to private hashmap
                                                                                     // of tickets customer owns
                            allFlights.get(inputID).addToProfit(newTotal);
                            System.out.println("Thank you for your purchase");
                            addToList("Customer " + allCustomers.get(importantInfo[1]).getFirstName() + " "
                                    + allCustomers.get(importantInfo[1]).getLastName() + " purchased "
                                    + ticketsPurchased + " First Class Tickets for Flight #" + inputID);
                    ...
                    System.out.println(
                            "As an Employee of MinerAirlines you get 50% off your First Class Ticket Purchase.\n");
                    ...
                    System.out.println(
                            "Your total comes out to: " + newTotal + " Would you like to proceed?\n\n1. Yes\n2. No\n");
                    int confirmPurchase = scan.nextInt();
                    if (allEmployees.get(importantInfo[1]).hasEnoughMoney(newTotal) && confirmPurchase == 1) { // This
                                                                                                               // is
                                                                                                               // used
                                                                                                               // to
                                                                                                               // check
                        ...
                        if (allFlights.get(inputID).checkSeats("First Class", ticketsPurchased,
                            ...
                            allFlights.get(inputID).addTicket(newTicket); // add ticket to public variable allTickets
                                                                          // which contains all purchased tickets
                            allEmployees.get(importantInfo[1]).addTicket(newTicket); // Add tickets to private hashmap
                                                                                     // of tickets customer owns

                            System.out.println("Thank you for your purchase");
                            addToList("Employee " + allEmployees.get(importantInfo[1]).getFirstName() + " "
                                    + allEmployees.get(importantInfo[1]).getLastName() + " purchased "
                                    + ticketsPurchased + " First Class Tickets for Flight #" + inputID);
                            newTicket.printTicket();
                        } else {
                            System.out.print(
                ...
                if (importantInfo[0].equalsIgnoreCase("Customer")) {
                    ...
                    if (allCustomers.get(importantInfo[1]).hasEnoughMoney(newTotal) && confirmPurchase == 1) { // This
                                                                                                               ...
                                                                                                               // enough
                                                                                                               // money
                        if (allFlights.get(inputID).checkSeats("Business Class", ticketsPurchased,
                                allFlights.get(inputID))) { // if theres enough seats avaailable then proceed
                            confirmationNumber += 1;
                            String confirmNumString = String.valueOf(confirmationNumber);
                            Ticket newTicket = new Ticket(newTotal, ticketsPurchased, confirmNumString,
                                    allFlights.get(inputID).getID(), importantInfo[1], "Business Class", true); // Create
                                                                                                                // a new
                                                                                                                // ticket
                                                                                                                // given
                                                                                                                // the
                            ...
                            addToList("Customer " + allCustomers.get(importantInfo[1]).getFirstName() + " "
                                    + allCustomers.get(importantInfo[1]).getLastName() + " purchased "
                                    + ticketsPurchased + " Business Class Tickets for Flight #" + inputID);
                            newTicket.printTicket();
                        } else {
                            System.out.print(
                                    "\nYou purchased more tickets than there are available, please enter another amount.");
                    ...
                    System.out.println(
                            "As an Employee of MinerAirlines you get 75% off your Business Class Ticket Purchase.\n");
                ...
                if (importantInfo[0].equalsIgnoreCase("Customer")) {
                    ...
                    if (allCustomers.get(importantInfo[1]).hasEnoughMoney(newTotal) && confirmPurchase == 1) { // This
                                                                                                               ...
                                                                                                               // enough
                                                                                                               // money
                        if (allFlights.get(inputID).checkSeats("Main Cabin", ticketsPurchased,
                                allFlights.get(inputID))) { // if theres enough seats avaailable then proceed
                            confirmationNumber += 1;
                            String confirmNumString = String.valueOf(confirmationNumber);
                            Ticket newTicket = new Ticket(newTotal, ticketsPurchased, confirmNumString,
                                    allFlights.get(inputID).getID(), importantInfo[1], "Main Cabin", true); // Create a
                                                                                                            // new
                                                                                                            // ticket
                                                                                                            // given the
                                                                                                            // flighst
                            ...
                            addToList("Customer " + allCustomers.get(importantInfo[1]).getFirstName() + " "
                                    + allCustomers.get(importantInfo[1]).getLastName() + " purchased "
                                    + ticketsPurchased + " Main Cabin Tickets for Flight #" + inputID);
                            newTicket.printTicket();
                        } else {
                            System.out.print(
                                    "\nYou purchased more tickets than there are available, please enter another amount.");
                    ...
                    System.out.println(
                            "As an Employee of MinerAirlines you get 75% off your Main Cabin Ticket Purchase.\n");
                    double discount = ticketTotal * 0.75;
                    double newTotal = ticketTotal - discount;
                    newTotal = addAllFees(ticketTotal, allFlights.get(inputID));
                    newTotal = minerAirlinesMember(allCustomers, allFlights, allEmployees, importantInfo, newTotal,
            ...
            if (actions[6].equals("First Class")) {
                ...
                if (importantInfo[0].equalsIgnoreCase("Customer")) {
                    ...
                    newTotal = minerAirlinesMember(allCustomers, allFlights, allEmployees, importantInfo, newTotal,
                            allFlights.get(inputID));
                    if (allCustomers.get(importantInfo[1]).hasEnoughMoney(newTotal)) { // This is used to check if the
                    ...
                    newTotal = minerAirlinesMember(allCustomers, allFlights, allEmployees, importantInfo, newTotal,
                            allFlights.get(inputID));
                    if (allEmployees.get(importantInfo[1]).hasEnoughMoney(newTotal)) { // This is used to check if the
                                                                                       // customer has enough money
                        if (allFlights.get(inputID).checkSeats("First Class", ticketsPurchased,
                                allFlights.get(inputID))) { // if theres enough seats avaailable then proceed
                            confirmationNumber += 1;
    ...
    public static boolean checkForInternational(Flight currFlight) {
        if (currFlight.getFlightType().equalsIgnoreCase("International")) {
            System.out.println("Since your Flight is International a " + currFlight.getSurcharge()
    ...
    public static void populateMaps(HashMap<String, Customer> allCustomers, HashMap<String, Flight> allFlights,
            HashMap<String, Employee> allEmployees) throws FileNotFoundException {
        ...
        Scanner customerScan = new Scanner(new File("CustomerListPA4.csv")); // scanner for file containing customer
                                                                             // data
        FileFactory fileContentOrganizor = new FileFactory(); // This is a class used to make contents in a file
                                                              // readable dynamically
        fileContentOrganizor.generateHashMapForFlightFile(); // calling on this will create a hashmap with the indexes
                                                             // of keywords inside file
        ...
        while (flightScan.hasNextLine()) { // This while loop populates allFlight hash map with all flight data
            ...
            Flight currFlight = fileContentOrganizor.generateFlightObject(flightInfo); // Using this method I look up
                                                                                       // indexes inside hashmap and
                                                                                       // place those ityems in the smae
                                                                                       // place ecerytime given their
                                                                                       // duynamically changing index
        ...
        while (customerScan.hasNextLine()) { // This while loop populates allCustomer hash map with all customer data
            ...
            if (clientInfo[3].equalsIgnoreCase("Customer")) { // check whetehr client is a customer
                ...
                allEmployees.put(currEmployee.getUserName(), currEmployee); /// This creates an entry in hasmap with
                                                                            /// username as key and info as value
            }
        }
    ...
    public static void cancelTicketsForFlight(Flight currFlight, HashMap<String, Customer> customerList) {
        ...
        for (String key : tempMap.keySet()) {
            customerList.get(tempMap.get(key).getUserName())
                    .setTotalMoney(customerList.get(tempMap.get(key).getUserName()).getTotalMoney()
                            + tempMap.get(key).getTotalPrice());
    ...
    public static String[] loginScreen(HashMap<String, Customer> allCustomers, HashMap<String, Flight> allFlights,
            HashMap<String, Employee> allEmployees) {
        Scanner scan = new Scanner(System.in);
        boolean loggedIn = false;
        String[] importantInfo = new String[] { "Role", "temp" };
        while (loggedIn != true) { // This is the main loop, here we start with username and password prompts then
                                   // continue to eveyrthing else
            ...
            while (isValid != true) {
                try {
                    if (allCustomers.get(userName).getFirstName().equalsIgnoreCase(firstName)
                            && allCustomers.get(userName).getLastName().equalsIgnoreCase(lastName)) {
                        System.out.println("First and Last name associated with username match up, you may proceed.");
                        System.out.println();
                        isValid = true;
                    } else {
                        System.out.println(
                                "First or Last name entered do not match up with username, please make sure you enetered the information correctly.");
                ...
                try {
                    if (allEmployees.get(userName).getFirstName().equalsIgnoreCase(firstName)
                            && allEmployees.get(userName).getLastName().equalsIgnoreCase(lastName)) {
                        System.out.println("First and Last name associated with username match up, you may proceed.");
                        System.out.println();
                        isValid = true;
                    } else {
            ...
            if (allCustomers.containsKey(userName) || allEmployees.containsKey(userName)) { // first check if username
                                                                                            // exists, if username
                                                                                            // doesnt exists then you
                                                                                            // will start from the
                                                                                            // beginning
    ...
    public static void customerMenu(String[] importantInfo, HashMap<String, Flight> allFlights,
            HashMap<String, Customer> allCustomers, HashMap<String, Employee> allEmployees) {
        String line = "------------------------------------------------------";

        System.out.println("");
        System.out.println("\t" + line);
        System.out
                .println("Our Records indicate youe are a Customer, Please locate your flights ID and enter it below.");
        System.out.println("\t" + line);
        System.out.println();
        ...
        while (validID == false) {
            ...
            if (validID == false) {
                System.out.print(
                        "Either the number you eneterd is invalid or the flight you are looking for does not exist. Please try again: ");
    ...
    public static void employeeMenu(String[] importantInfo, HashMap<String, Customer> allCustomers,
            HashMap<String, Flight> allFlights, HashMap<String, Employee> allEmployees) throws FileNotFoundException {
        String line = "------------------------------------------------------";
        Scanner scan = new Scanner(System.in);
            ...
            if (employeeMenuSelection1.equalsIgnoreCase("1")) { // this is to view flight information
                ...
                switch (employeeMenuSelection2) {
                    case "1":
                        ...
                        break;
                    case "2":
                        System.out.println("The Total number of seats remaining is: "
                                + allFlights.get(employeeFlightSelection).getTotalSeats());
                        break;
                    case "3":
                        System.out.println("The number of First Class Seats remaining is: "
                                + allFlights.get(employeeFlightSelection).getFirstClassSeats());
                        break;
                    case "4":
                        System.out.println("The number of Business Class Seats remaining is: "
                                + allFlights.get(employeeFlightSelection).getBusinessClassSeats());
                        break;
                    case "5":
                        System.out.println("The number of Main Cabin Seats remaining is: "
                                + allFlights.get(employeeFlightSelection).getMainCabinSeats());
                        break;
                    case "6":
                        System.out.println("The customers on this flight are as follows: ");
                    case "7":
                        System.out.println("The total amount collected from First Class ticket sales is: "
                                + allFlights.get(employeeFlightSelection).totalAmountFirstClass());
                    case "8":
                        System.out.println("The total amount collected from Business Class ticket sales is: "
                                + allFlights.get(employeeFlightSelection).totalAmountBusinessClass());
                    case "9":
                        System.out.println("The total amount collected from Main Cabin ticket sales is: "
                                + allFlights.get(employeeFlightSelection).totalAmountMainCabin());
                    case "10":
                        double totalTicketSales = allFlights.get(employeeFlightSelection).totalAmountFirstClass()
                                + allFlights.get(employeeFlightSelection).totalAmountBusinessClass()
                                + allFlights.get(employeeFlightSelection).totalAmountMainCabin();
                    ...
                    case "11":
                        ...
                        System.out
                                .println("The expected profit to be collected from this flight is: " + expectedProfit);
                    case "12":
                        System.out.println("Total Sceutorty Fee collected for Flight " + employeeFlightSelection + ": "
                                + allFlights.get(employeeFlightSelection).getSecurityFee());
                    case "13":
                        System.out.println("Total Miner Airlines Fee collected for Flight " + employeeFlightSelection
                                + ": " + allFlights.get(employeeFlightSelection).getMinerAirlinesFee());
                    case "14":
                        System.out.println("Total Tax collected for Flight " + employeeFlightSelection + ": "
                                + allFlights.get(employeeFlightSelection).getTaxCollected());
                    default:
    ...
    public static void main(String[] args) throws IOException {
        ...
        // these are the hashmaps that are used to view and access relative infromation
        // to these objects

        populateMaps(allCustomers, allFlights, allEmployees); // Running this will populate all of the hash maps with
                                                              // information from csv files
        Scanner scan = new Scanner(System.in);
        // File write to write contents to a separte file
        FileWriter flightUpdateLog = new FileWriter("flightUpdateLog.txt", true); // this is used to write actions to a
                                                                                  // separate file
        boolean loop = true;
        while (loop == true) {
            // This is the main loop, here we start with username and password prompts then
            // continue to eveyrthing else //if username exists check if password is valid,
            // now select flight. If pass is incorrect then you will restart from the
            // beginning
        ...
        for (String str : allStrings) { // This writes eeveything form yhe book intot he none
                                        // eixitygkkldslklsdksklj;saklkkdklksal;;a akak akak kakkka a kekewkskKALSKA
                                        // SKlklsk k wkl;kalks KSKSKL;AKlkslak
